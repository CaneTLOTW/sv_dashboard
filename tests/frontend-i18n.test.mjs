...
test("runtime wires all advanced catalogs with the release cache key", () => {
  ...
  assert.match(runtimeSource, /i18n-core\.js\?v=0\.6\.0-beta\.27/);
});
...
test("dual-energy hero follows the shared 18-language runtime contract", () => {
  ...
  for (const key of ["battery", "fuel", "chargePower", "fuelConsumption", "charging", "driving", "plugged"]) {
    assert.match(dualEnergy, new RegExp(`text\\.${key}`), `dual energy card does not consume ${key}`);
  }
  assert.match(dualEnergy, /dashboardText\.range/);
  assert.doesNotMatch(dualEnergy, /text\.(electricRange|fuelRange)/);
