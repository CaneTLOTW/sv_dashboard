...
test("Home Assistant registers one SV frontend resource", () => {
  ...
  assert.match(constants, /FRONTEND_VERSION = "0\.5\.54"/);
  assert.match(constants, /FRONTEND_RESOURCE_URLS = \(FRONTEND_URL,\)/);
  assert.match(frontend, /import\("\.\/vehicle-overview-card\.js\?v=0\.5\.54"\)/);
  assert.match(frontend, /import\("\.\/gps-history-card\.js\?v=0\.5\.54"\)/);
  assert.match(frontend, /import\("\.\/sv_dashboard\.js\?v=0\.5\.54"\)/);
...
test("dependency preflight waits instead of failing on first customElements lookup", () => {
  ...
  assert.match(frontend, /await import\("\.\/sv_dashboard\.js\?v=0\.5\.54"\)/);
});
